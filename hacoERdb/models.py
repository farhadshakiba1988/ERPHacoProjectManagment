...
class City(models.Model):
    ...
    meta = JSONField(default=dict)
    parent_city = models.ForeignKey('self', related_name='child_cities', null=True, blank=True,
...
class Company(models.Model):
    ...
    company_type = models.CharField(max_length=50, default='')
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def get_applied_jobs(self):
        ids = AppliedJobs.objects.filter(user=self).exclude(ip_address='', user_agent='').values_list('job_post',
                                                                                                      flat=True)
    ...
    def get_subscribed_skills(self):
        ...
        return Skill.objects.filter(
            id__in=Subscriber.objects.filter(email__in=user_emails, is_verified=True).values_list('skill',
                                                                                                  flat=True)).distinct()
    ...
    def related_jobs(self):
        ...
        related_jobs = JobPost.objects.filter(
            Q(status='Live') & Q(skills__in=user_skills) | Q(location__in=[self.current_city])).exclude(id__in=ids)
        related_jobs = list(related_jobs) + list(JobPost.objects.filter(status='Live').order_by('-published_on')[:15])
...
class StackOverFlow(models.Model):
    user = models.ForeignKey(User, related_name='stackoverflow', on_delete=models.CASCADE)
...
class JobPost(models.Model):
    ...
    agency_invoice_type = models.CharField(max_length=50, default='Recurring')
    ...
    def get_absolute_url(self):
        qs = self.title.replace(
            '/[^a-zA-Z-]/g', "").title().strip().strip('.')
        qs = qs.replace(" ", '-').lower()
        ...
        if str(self.job_type) == 'internship':
            ...
            qs = '/' + qs + '-' + \
                 str(self.min_year) + '-to-' + str(self.max_year) + \
                 '-years-' + str(self.id) + '/'
        return qs

    def get_job_minified_url(self):
        job_url = '' + self.get_absolute_url()
        return job_url
