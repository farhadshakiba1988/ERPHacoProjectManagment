...
from hacoERdb.models import JobPost, Skill, City, FunctionalArea, Industry, Qualification, State
from datetime import datetime
from django.core import serializers


class jobIndex(indexes.SearchIndex, indexes.Indexable):
    text = indexes.CharField(document=True, use_template=True, template_name="index/job_text.txt")
    ...
    def prepare_created_on(self, obj):
        if obj.created_on:
            current_date = datetime.strptime(str(obj.created_on), "%Y-%m-%d").strftime("%Y-%m-%d")
            return current_date
        return None

    def prepare_published_on(self, obj):
        if obj.published_on:
            current_date = datetime.strptime(str(obj.published_on.date()), "%Y-%m-%d").strftime("%Y-%m-%d")
            return current_date
        return None
    ...
    def index_queryset(self, using=None):
        ...
        return self.get_model().objects.filter(status='Live').select_related(
            'company', 'user'
        ).prefetch_related('location', 'edu_qualification', 'industry', 'skills', 'functional_area')


class skillautoIndex(indexes.SearchIndex, indexes.Indexable):

    text = indexes.CharField(document=True, use_template=True, template_name="index/skill_text.txt")
...
class locationIndex(indexes.SearchIndex, indexes.Indexable):
    text = indexes.CharField(document=True, use_template=True, template_name="index/city_text.txt")
...
class industryIndex(indexes.SearchIndex, indexes.Indexable):
    text = indexes.CharField(document=True, use_template=True, template_name="index/industry_text.txt")
...
class functionalareaIndex(indexes.SearchIndex, indexes.Indexable):
    text = indexes.CharField(document=True, use_template=True, template_name="index/functionalarea_text.txt")
...
class qualificationIndex(indexes.SearchIndex, indexes.Indexable):
    text = indexes.CharField(document=True, use_template=True, template_name="index/qualification_text.txt")
...
class stateIndex(indexes.SearchIndex, indexes.Indexable):
    text = indexes.CharField(document=True, use_template=True, template_name="index/state_text.txt")
