...
from hacoERdb.models import (User, Country, State, City, Skill,
                           Qualification, Industry, FunctionalArea, JobPost,
                           InterviewLocation)
...
class BaseTest(TestCase):
    ...
    def setUp(self):
        ...
        self.recruiter = User.objects.create(
            email='recruiter@mp.com', username="recruiter", user_type='RR', is_active=True, mobile_verified=True)
        ...
        self.admin_user = User.objects.create(
            email='mp@mp.com', username="mp@mp.com")
        ...
        for each in range(0, 15):
            self.jobpost = JobPost.objects.create(user=self.user, country=self.country,
                                                  title='test-jobpost_' +
                                                  str(each),
            ...
            self.interview_location = InterviewLocation.objects.create(venue_details='hyderabad, India',
                                                                       latitude='14.8976',
                                                                       longitude='21.0967')
        ...
        for each in range(0, 15):
            self.jobpost = JobPost.objects.create(user=self.user, country=self.country,
                                                  ...
                                                  str(each),
                                                  vacancies='6', description='job post description',
                                                  job_type='internship', status='Live',
                                                  published_message='test message',
                                                  company_address='company address',
                                                  company_description='company description',
                                                  last_date=current_date)
...
class all_views_get_for_job_seeker(BaseTest):
    ...
    def test_views_get(self):
        ...
        response = self.client.get(reverse('jobs_by_location', kwargs={'job_type': 'jobs'}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'jobs/jobs_by_location.html')

        response = self.client.get(reverse('jobs_by_location', kwargs={'job_type': 'jobs'}) + '?page=1')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'jobs/jobs_by_location.html')

        response = self.client.get(reverse('jobs_by_location', kwargs={'job_type': 'jobs'}) + '?page=100')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'jobs/jobs_by_location.html')
        ...
        response = self.client.get(
            reverse('city_internship_jobs', kwargs={'location': self.city.slug}) + '?page=1')
        self.assertEqual(response.status_code, 301)
        # self.assertTemplateUsed(response, 'jobs/jobs_list.html')

        response = self.client.get(
            reverse('city_internship_jobs', kwargs={'location': self.city.slug}) + '?page=100')
        self.assertEqual(response.status_code, 301)
        # self.assertTemplateUsed(response, 'jobs/jobs_list.html')
        ...
        response = self.client.get(
            reverse('job_industries', kwargs={'industry': self.industry.slug}) + '?page=1')
        self.assertEqual(response.status_code, 301)
        # self.assertTemplateUsed(response, 'jobs/jobs_list.html')

        response = self.client.get(
            reverse('job_industries', kwargs={'industry': self.industry.slug}) + '?page=100')
        self.assertEqual(response.status_code, 301)
        response = self.client.get(
        ...
        response = self.client.get(
            reverse('pages', kwargs={'page_name': 'about-us'}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'pages/about-us.html')

        response = self.client.get(
            reverse('pages', kwargs={'page_name': 'about'}))
        self.assertEqual(response.status_code, 404)
        self.assertTemplateUsed(response, '404.html')
...
class calendar_views_get(BaseTest):
    ...
    def test_views_get(self):
        ...
        response = self.client.get(reverse('month_calendar',
                                   kwargs={'year': datetime.now().year,
                                           'month': datetime.now().month}))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "calendar/year_calendar.html")

        response = self.client.get(reverse('week_calendar',
                                   kwargs={'year': datetime.now().year,
                                           'month': datetime.now().month,
                                           'week': '1'}))
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(reverse('jobposts_by_date', kwargs={'year': datetime.now(
        ).year, 'month': datetime.now().month, 'date': datetime.now().date().day}))
        self.assertEqual(response.status_code, 200)
